...
namespace DuiLib {
	...
	LRESULT CMenuWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
	{
		...
		case WM_KEYDOWN:
		{
			...
			{
				OutputDebugString(_T("aaaaaaaaa VK_LEFT VK_RIGHT \n"));
				::PostMessage(m_hParentWnd, WM_USER + 2049, wParam, 0L);
			}
			break;
		}
		case WM_SIZE:
	...
	void CMenuElementUI::DoEvent(TEventUI& event)
	{
		if (event.Type == UIEVENT_SYSKEY)
		{
			...
			{
				CControlUI* pControl = static_cast<CControlUI*>( CMenuWnd::GetGlobalContextMenuObserver().GetManager()->FindControl(_T("root")));
				CMenuWnd::GetGlobalContextMenuObserver().GetManager()->SendNotify(pControl, _T("alt"), 18, 0);
			}
			return;
		}
