...
namespace DuiLib
{
	...
	LRESULT WindowImplBase::OnSysCommand(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
	{
		...
		if( ::IsZoomed(*this) != bZoomed ) {
			if( !bZoomed ) {
				...
				OnMainWndMaxMinEx(true);

				MONITORINFO oMonitor = {};
				oMonitor.cbSize = sizeof(oMonitor);
				::GetMonitorInfo(::MonitorFromWindow(*this, MONITOR_DEFAULTTOPRIMARY), &oMonitor);
				double x = 160.0, y = 43.0, x1 = 1300, y1 = 750;
				int width = (int)(x / x1 * (oMonitor.rcWork.right - oMonitor.rcWork.left));
				int height = (int)(y / y * (oMonitor.rcWork.bottom - oMonitor.rcWork.top));
				CButtonUI* pBtnBodyLeft = static_cast<CButtonUI*>(m_pm.FindControl(_T("bodyleft")));
				pBtnBodyLeft->SetFixedWidth(width);
				pBtnBodyLeft->SetFixedHeight(height);
			}
			else {
				CControlUI* pControl = static_cast<CControlUI*>(m_pm.FindControl(_T("maxbtn")));
				if( pControl ) pControl->SetVisible(true);
	...
	BOOL WindowImplBase::OnMainWndMaxMinEx(bool bMax)
	{
		return TRUE;
	}
